...
const DynamicBackground = ({ logoPath = "/images/logos/whiteFavicon.png" }) => {
  ...
  useEffect(() => {
      ...
      canvasBg:
        typeof window !== "undefined"
          ? getComputedStyle(document.documentElement)
              ...
              .trim() || "#783293"
          : "#783293",
      logoSize: isMobileRef.current ? 700 : 1100,
      distortionRadius: isMobileRef.current ? 1500 : 3000,
      forceStrength: 0.003,
      maxDisplacement: isMobileRef.current ? 50 : 100,
      returnForce: 0.025,
      pixelStep: isMobileRef.current ? 2 : 1,
    };
    ...
    function initParticleSystem(pixels, dim) {
      ...
      for (let i = 0; i < dim; i += CONFIG.pixelStep) {
        for (let j = 0; j < dim; j += CONFIG.pixelStep) {
    ...
    const handleResize = () => {
      ...
      if (geometryRef.current && particleGridRef.current.length > 0) {
        ...
        for (let i = 0; i < particleGridRef.current.length; i++) {
          ...
          const newX = centerX + particle.offsetX;
          const newY = centerY + particle.offsetY;
